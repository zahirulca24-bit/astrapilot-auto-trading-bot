...
type Options = { url?: string; heartbeatMs?: number; reconnectBaseMs?: number; reconnectMaxMs?: number; websocketFactory?: WsFactory; now?: () => number; random?: () => number };
...
export class BybitPublicWebSocketProvider implements PublicMarketDataProvider {
  ...
  async subscribe(symbols: string[]): Promise<void> { symbols.forEach((s) => this.symbols.add(s)); this.sendSubscription('subscribe', symbols); }
  async unsubscribe(symbols: string[]): Promise<void> { symbols.forEach((s) => this.symbols.delete(s)); this.sendSubscription('unsubscribe', symbols); }

  status(): Omit<GatewayStatus,'service'|'version'|'staleAfterMs'> {
    return { state:this.state, provider:this.name, publicDataOnly:true, exchangeCredentialsAccepted:false, lastEventAt:this.lastEventAt, symbols:[...this.symbols].sort(), subscriptions:this.symbols.size, reason:this.state === 'connected' ? null : `WebSocket state: ${this.state}` };
  }
  ...
  private connect(): void {
    ...
    ws.on('open', () => {
      this.state = 'connected'; this.reconnectAttempt = 0;
