...
def _health_service(request: Request, settings: Settings) -> HealthService:
    ...
    return HealthService(settings, database_manager, startup_error)
...
def readiness(
    ...
    service = _health_service(request, settings)
...
def health(
    ...
    return HealthResponse(
        service=settings.app_name,
        status=health_status if ready else HealthState.DEGRADED,
        timestamp=service.now(),
        version=settings.app_version,
